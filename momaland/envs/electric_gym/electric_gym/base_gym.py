...
class BaseElectricGym(MOParallelEnv, EzPickle):
    ...
    def __init__(
        ...
        base_observation_space["line_loading"] = spaces.Box(low=0, high=1, shape=(1,))
        base_observation_space["transformer_loading"] = spaces.Box(low=0, high=1, shape=(1,))

        base_observation_space = Dict(base_observation_space)
        ...
        self.reward_spaces = {agent: base_reward_space for agent in self.possible_agents}
        ...
        price_path = os.path.join(os.path.dirname(__file__), "./grid_model/data/spotmarket_reduced_quarters_2023.csv")
    ...
    def reset(self, seed=None, options=None):
        ...
        self.rewards = {agent: self._get_rewards(agent) for agent in self.agents}
    ...
    def _get_observation(self, agent):
        ...
        ev_soc_percent = self.gridMgr.get_bus_ev_soc(agent)
        ...
        base_observation = {
            "pv_power": np.array(pv_power.values.flatten(), dtype=np.float32),
            "load_power": np.array(load_power.values.flatten(), dtype=np.float32),
            "heating_power": np.array(hp_power.values.flatten(), dtype=np.float32),
    ...
    def _get_ev_flexibility_reward(self, agent):
        ...
        return total_flexibility_margin

    def _get_voltage_reward(self, bus):
        """
        Returns a cost based on the voltage deviation from the nominal value.
        """
        voltage = self.gridMgr.get_bus_voltage_pu(bus)

        voltage_deviation = (1 - voltage) ** 2

        voltage_cost = self._normalize(voltage_deviation, 0, 0.01)
    ...
    def _take_action(self, action):
        ...
        """
        random_numbers = self.np_random.random((5, 3))
